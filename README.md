...
This repository contains the frontend experience, representative review states, and a conservative Burmese text-quality layer. The detection layer combines Unicode-structure checks with a curated core lexicon, an approved-uncommon-word allowlist, and categorized legal and medical terminology. Domain terms can be enabled per document type, so valid specialist terms are recognized without suppressing malformed syllables. See [lexicon-sources.md](./lexicon-sources.md) for the reference and scope notes. Production OCR, Burmese language-model analysis, document persistence, and corrected DOCX/PDF export integrations are intentionally not connected yet.
