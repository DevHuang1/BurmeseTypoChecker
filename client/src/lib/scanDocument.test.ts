...
describe("scanBurmeseDocument", () => {
  it("maps a structural Burmese issue to a line, character, and page", () => {
    ...
    expect(finding).toMatchObject({ code: "DUPLICATE_VOWEL_MARK", page: 1, line: 2, character: 2, confidence: "99%" });
  });
  ...
  it("finds obvious repeated spaces without calling unknown vocabulary a typo", () => {
    ...
    expect(finding).toMatchObject({ code: "EXTRA_SPACE", type: "Repeated space", confidence: "96%" });
  });
});
