...
export default function Home() {
  ...
  const onFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!nextFile) return;
    setFile(nextFile); setSourceText(""); setFindings([]); setSelectedFindingId(null); setStage("ready"); setError(null);
  ...
  const startScan = async () => {
    ...
    try {
      ...
      setSourceText(text); setStage("scanning");
      window.setTimeout(() => {
        const nextFindings = scanBurmeseDocument(text);
        setFindings(nextFindings); setSelectedFindingId(nextFindings[0]?.id ?? null); setStage("complete");
        window.setTimeout(() => resultsRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 40);
        toast.success("Scan complete", { description: nextFindings.length ? `${nextFindings.length} high-confidence finding${nextFindings.length === 1 ? "" : "s"} mapped to source locations.` : "No high-confidence structural typos found." });
      }, 420);
