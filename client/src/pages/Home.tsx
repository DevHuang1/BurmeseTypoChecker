...
import { getFindingHighlightRange } from "@/lib/findingHighlight";
...
function HighlightedFindingExcerpt({ sourceText, finding, className = "" }: { sourceText: string; finding: ScanFinding; className?: string }) {
  const { chars, start, end, markStart, markEnd } = getFindingHighlightRange(sourceText, finding.index, finding.length);
  const localStart = markStart - start;
  const localEnd = markEnd - start;
  const before = chars.slice(start, start + localStart).join("");
  const marked = chars.slice(start + localStart, start + localEnd).join("");
  const after = chars.slice(start + localEnd, end).join("");
  return <p className={`burmese leading-[1.9] ${className}`}><span className="text-[#324258]">{start > 0 ? "…" : ""}{before}</span><mark data-finding-highlight="true" className="rounded-md bg-[#f2b36d] px-1 py-0.5 text-[#24354f] shadow-[0_0_0_2px_rgba(242,179,109,.25)]">{marked || " "}</mark><span className="text-[#324258]">{after}{end < chars.length ? "…" : ""}</span></p>;
}

function HighlightedExtractedDocument({ sourceText, finding }: { sourceText: string; finding?: ScanFinding }) {
  if (!finding) return <p className="burmese whitespace-pre-wrap text-[17px] leading-[2.05] text-[#3c4c5f]">{sourceText}</p>;
  const chars = safeChars(sourceText);
  const { markStart, markEnd } = getFindingHighlightRange(sourceText, finding.index, finding.length, 0, 0);
  const nodes: React.ReactNode[] = [];
  let cursor = 0;
  if (markStart > 0) nodes.push(<span key="before">{chars.slice(0, markStart).join("")}</span>);
  nodes.push(<mark key="finding" data-finding-highlight="true" className="rounded-md bg-[#f2b36d] px-1 py-0.5 text-[#24354f] shadow-[0_0_0_2px_rgba(242,179,109,.25)]">{chars.slice(markStart, markEnd).join("") || " "}</mark>);
  cursor = markEnd;
  if (cursor < chars.length) nodes.push(<span key="after">{chars.slice(cursor).join("")}</span>);
  return <p className="burmese whitespace-pre-wrap text-[17px] leading-[2.05] text-[#3c4c5f]">{nodes}</p>;
}

function ScanResults({ findings, sourceText, onSelect }: { findings: ScanFinding[]; sourceText: string; onSelect: (id: string) => void }) {
