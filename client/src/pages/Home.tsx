...
function ScanResults({ findings, onSelect }: { findings: ScanFinding[]; onSelect: (id: string) => void }) {
  ...
  return <section aria-live="polite" className="mt-5 overflow-hidden rounded-[22px] border border-[#ded6c9] bg-[#fbf8f0] shadow-[0_10px_24px_rgba(50,44,34,.05)]"><div className="flex flex-col gap-2 border-b border-[#e5ddd1] px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><div className="flex items-center gap-2 text-sm font-bold"><span className="h-px w-5 bg-[#c85a3f]" /> Instant scan results <span className="rounded-full bg-[#f2d5cb] px-2 py-0.5 text-[10px] font-bold text-[#a24d3b]">{findings.length} found</span></div><div className="mt-1 text-[11px] text-[#8e9699]">Typos are mapped to extracted page, line, and character locations.</div></div><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.13em] text-[#6e9072]"><span className="h-1.5 w-1.5 rounded-full bg-[#6e9072]" /> Locations ready</div></div><div className="divide-y divide-[#e8dfd3]">{findings.map((finding) => <button key={finding.id} onClick={() => onSelect(finding.id)} className="flex w-full items-start gap-3 px-5 py-4 text-left transition hover:bg-[#f5eee4] sm:gap-4 sm:px-6"><div className="mt-0.5 flex shrink-0 items-center gap-1.5 rounded-lg bg-[#f3d8cf] px-2 py-1 text-[10px] font-bold text-[#a24d3b]"><MapPin size={12} />P{finding.page} · L{finding.line}</div><div className="min-w-0 flex-1"><div className="mb-1 flex flex-wrap items-center gap-2"><span className="text-[10px] font-bold uppercase tracking-[.14em] text-[#9a8e82]">{finding.type}</span><span className="text-[10px] font-semibold text-[#80908a]">Char {finding.character}</span><span className="text-[9px] font-bold text-[#648269]">{finding.confidence}</span></div><p className="burmese truncate text-[15px] leading-[1.8] text-[#324258]">{finding.excerpt}</p><p className="mt-1 truncate text-[12px] leading-[1.7] text-[#a15d49]"><span className="mr-1 font-bold uppercase tracking-widest text-[#b16b54]">Suggest</span>{finding.suggestion}</p></div><ArrowUpRight size={15} className="mt-1 shrink-0 text-[#abb0ae]" /></button>)}</div></section>;
}
...
export default function Home() {
  ...
  const onFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!nextFile) return;
    setFile(nextFile); setSourceText(""); setEditableText(""); setFindings([]); setSelectedFindingId(null); setStage("ready"); setError(null); setEditingExtractedText(false);
  ...
  const scanText = (text: string, message: string) => {
    ...
    window.setTimeout(() => {
      ...
      setFindings(nextFindings); setSelectedFindingId(nextFindings[0]?.id ?? null); setStage("complete"); setEditingExtractedText(false); setTab("review");
  ...
  const startScan = async () => {
    ...
    setError(null); setFindings([]); setSelectedFindingId(null); setStage("extracting");
    try {
    ...
    } catch (scanError) { const message = scanError instanceof Error ? scanError.message : "The file could not be scanned."; setError(message); setStage("error"); toast.error("Scan could not complete", { description: message }); }
  };
